...
let menusStack = [];
class BrowserDetector{constructor(){this.browser={},this.unsupportedBrowsers={Chrome:70,Firefox:60,IE:10,Edge:15,Opera:50,Safari:12},this._detectBrowser()}_detectBrowser(){var e,r,s;this.browser=(r=navigator.userAgent,s=r.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i)||[],/trident/i.test(s[1])?{name:"IE",version:(e=/\brv[ :]+(\d+)/g.exec(r)||[])[1]||""}:"Chrome"===s[1]&&null!=(e=r.match(/\b(OPR|Edge)\/(\d+)/))?{name:e[1].replace("OPR","Opera"),version:e[2]}:(s=s[2]?[s[1],s[2]]:[navigator.appName,navigator.appVersion,"-?"],null!=(e=r.match(/version\/(\d+)/i))&&s.splice(1,1,e[1]),{name:s[0],version:s[1]}))}get isIE(){return"IE"===this.browser.name}get isEdge(){return"Edge"===this.browser.name}get isMicrosoft(){return this.isIE||this.isEdge}get isFirefox(){return"Firefox"===this.browser.name}get isChrome(){return"Chrome"===this.browser.name}get isSafari(){return"Safari"===this.browser.name}get isAndroid(){return/Android/i.test(navigator.userAgent)}get isBlackBerry(){return/BlackBerry/i.test(navigator.userAgent)}get isWindowsMobile(){return/IEMobile/i.test(navigator.userAgent)}get isIOS(){return/iPhone|iPad|iPod/i.test(navigator.userAgent)}get isMobile(){return this.isAndroid||this.isBlackBerry||this.isWindowsMobile||this.isIOS}isSupported(){return!!(this.unsupportedBrowsers.hasOwnProperty(this.browser.name)&&+this.browser.version>this.unsupportedBrowsers[this.browser.name])}}
function load() {
...
function generateBackgroundParticles(q = 7){
    setTimeout(() => {
        for (let i = 0; i < q; i++) {
            let x = document.createElement("img");
